...
def dilated_net(x, drop_probability, is_training):
    with tf.variable_scope('dilated_net'):
        ...
        """ dilated 8x8"""
        dilated1 = tf.layers.conv2d(inputs=pool5,
        ...
        """ dilated 8x8"""
        dilated2 = tf.layers.conv2d(inputs=tf.concat([pool5, batch_dlt1], axis=3),
                                    filters=128, kernel_size=[3, 3], strides=[1, 1], padding='same',
        ...
        """ dilated 8x8"""
        dilated3 = tf.layers.conv2d(inputs=tf.concat([pool5, batch_dlt1, batch_dlt2], axis=3),
                                    filters=128, kernel_size=[3, 3], strides=[1, 1], padding='same',
        ...
        """ dilated 8x8"""
        dilated4 = tf.layers.conv2d(inputs=tf.concat([pool5, batch_dlt1, batch_dlt2, batch_dlt3], axis=3),
                                    filters=128, kernel_size=[3, 3], strides=[1, 1], padding='same',
        ...
        dilated_out = tf.concat([pool5, batch_dlt1, batch_dlt2, batch_dlt3, batch_dlt4], axis=3)

        """ deconv5 16x16"""
        deconv5 = tf.layers.conv2d_transpose(inputs=dilated_out, filters=512, kernel_size=[3, 3],
